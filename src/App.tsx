...
import { Search, Globe, Users, Award, CheckCircle, Send, PlayCircle, Clock, Star, BookOpen, Layers, Zap } from 'lucide-react';
...
interface Course {
  id: number;
  title: string;
  category: 'Freelancing' | 'Trading' | 'Skills' | 'Marketing' | 'E-commerce' | 'Design' | 'Business' | 'Data Science';
...
const LIVE_COURSES: Course[] = [
  { id: 1, title: 'Complete Upwork Mastery 2025', category: 'Freelancing', link: 'https://forms.gle/xFQtUXY6ek9QLRiS7', duration: '12 Hours', instructor: 'Mike', students: '35K+' },
  { id: 2, title: 'ICT Forex Trading Full Course', category: 'Trading', link: 'https://www.youtube.com/watch?v=pM3zcoF1HbQ', duration: '18 Hours', instructor: 'Mike', students: '28K+' },
  { id: 3, title: 'Video Editing Full Course (CapCut/Premiere)', category: 'Skills', link: 'https://www.youtube.com/watch?v=NgrXxAPxmEY', duration: '15 Hours', instructor: 'Mike', students: '42K+' },
  { id: 4, title: 'Facebook Ads Full Course (Meta Ads)', category: 'Marketing', link: 'https://www.youtube.com/watch?v=PDHTln7QTxU', duration: '10 Hours', instructor: 'Mike', students: '30K+' },
  { id: 5, title: 'Dropshipping Zero to Hero', category: 'E-commerce', link: 'https://bit.ly/GETPRINTIFY', duration: '8 Hours', instructor: 'Mike', students: '20K+' },
  { id: 6, title: 'Python for Financial Analysis', category: 'Data Science', link: '#', duration: '20 Hours', instructor: 'Mike', students: '15K+' },
  { id: 7, title: 'The Modern Business Blueprint', category: 'Business', link: '#', duration: '25 Hours', instructor: 'Mike', students: '18K+' },
];

const CATEGORY_TABS = ['All', 'Freelancing', 'Trading', 'Skills', 'Marketing', 'E-commerce', 'Business', 'Data Science'];
...
const CTAButton: React.FC<{ children: React.ReactNode, link: string, className?: string }> = ({ children, link, className = "" }) => (
  <a 
    href={link} 
    target="_blank" 
...
const CourseCard: React.FC<{ course: Course }> = ({ course }) => {
    ...
    const getAccentColor = (category: string) => {
        switch (category) {
            case 'Freelancing': return 'green';
            case 'Trading': return 'red';
            case 'Skills': return 'blue';
            case 'Marketing': return 'pink';
            case 'E-commerce': return 'yellow';
    ...
    return (
        ...
        >
            ...
            <div className="relative h-40 mb-4 rounded-xl overflow-hidden flex items-center justify-center bg-gradient-to-br from-indigo-900 to-indigo-700">
                ...
                <div className={`w-32 h-32 bg-${accent}-500/20 rounded-full blur-xl absolute -bottom-10 -right-10`}></div>
                <div className={`w-20 h-20 bg-${accent}-500/40 rounded-lg absolute top-4 left-4 transform rotate-6`}></div>
...
const PartnerSection: React.FC = () => (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 md:mt-16">
        <h3 className="text-lg font-semibold text-gray-400 mb-6 text-center">Our Course Partners</h3>
        <div className="relative flex items-center justify-center">
            ...
            <div className="flex overflow-x-auto justify-start md:justify-center space-x-10 py-4 custom-scrollbar-partners">
                {PARTNERS.map((p, index) => (
                    ...
                    >
                        <div className={`w-10 h-10 flex items-center justify-center rounded-full bg-${p.color}-500/20 text-${p.color}-300 border border-${p.color}-500/50`}>
                            {p.logo}
                        </div>
...
const CategoryTabs: React.FC<{ selected: string, onSelect: (cat: Course['category'] | 'All') => void }> = ({ selected, onSelect }) => (
...
const Header: React.FC = () => (
    <header className="sticky top-0 z-50 bg-[#1A1A2E]/95 backdrop-blur-sm border-b border-[#3A3A60]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
            <a href="#home" className="text-2xl font-black text-white flex items-center">
            ...
            <nav className="hidden md:flex space-x-8 text-text-light font-medium">
                {['Home', 'Courses', 'Mentorship', 'Success Stories', 'Contact'].map((item) => (
                    <a key={item} href={`#${item.toLowerCase().replace(' ', '-')}`} className="hover:text-primary-purple transition duration-150">
                        {item}
                    </a>
                ))}
            </nav>
            <CTAButton link="https://t.me/etubersmoney" className="hidden md:block py-2 px-6 text-sm">
                <span className="flex items-center"><Send className="w-4 h-4 mr-2" /> Join Telegram</span>
            </CTAButton>
...
const HeroSection: React.FC = () => (
    <section id="home" className="relative pt-20 pb-24 md:pt-32 text-center overflow-hidden bg-gradient-to-br from-[#1A1A2E] to-[#1A1A4E]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 z-10 flex flex-col md:flex-row items-center justify-between text-left">
            ...
            <div className="w-full md:w-1/2 lg:w-3/5 text-center md:text-left mb-10 md:mb-0">
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-6 leading-tight tracking-tighter">
                    Best courses are waiting to <span className="text-primary-purple">enrich your skill</span>.
                </h1>
                <p className="text-lg md:text-xl text-text-light mb-10 font-light max-w-xl">
                    Provides you with the latest online learning system and material that help your knowledge grow.
                </p>
...
const CourseCatalogSection: React.FC = () => {
    ...
    const filteredCourses = LIVE_COURSES.filter(course => 
      selectedCategory === 'All' || course.category === selectedCategory
    );

    return (
        <section id="courses" className="py-20 md:py-28 bg-[#1A1A3E] border-t border-[#3A3A60]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h2 className="text-4xl font-extrabold text-center text-white mb-4">Popular Courses</h2>
                <p className="text-xl text-center text-text-light mb-12">Enroll today and start building your financial future.</p>
...
const TestimonialsSection: React.FC = () => (
    <section id="success-stories" className="py-16 md:py-24 bg-background-dark border-t border-[#3A3A60]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl md:text-4xl font-bold text-center text-white mb-4">Success Stories: Talk Less, Earn More</h2>
            <p className="text-xl text-center text-gray-400 mb-12">See how our students are changing their lives.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                ...
                <div className="p-6 bg-[#232345] rounded-xl border-l-4 border-accent-gold shadow-md">
                    ...
                    <p className="text-gray-300 italic mb-4">"Landed my first big Upwork gig thanks to Mike's mentorship. This isn't just theory, it's a blueprint for making money."</p>
                    <p className="text-sm font-semibold text-white">— Samuel K. (Ethiopian Freelancer)</p>
                </div>
                {/* Testimonial Card 2 */}
                <div className="p-6 bg-[#232345] rounded-xl border-l-4 border-accent-gold shadow-md">
                    <Star className="w-5 h-5 text-accent-gold mb-3" />
                    <p className="text-gray-300 italic mb-4">"The Forex course structure is incredibly clear. I moved from demo to live trading with confidence. Practical advice is spot on!"</p>
                    <p className="text-sm font-semibold text-white">— Almaz T. (ICT Trader)</p>
                </div>
                {/* Testimonial Card 3 */}
                <div className="p-6 bg-[#232345] rounded-xl border-l-4 border-accent-gold shadow-md">
                    <Star className="w-5 h-5 text-accent-gold mb-3" />
                    <p className="text-gray-300 italic mb-4">"The video editing course opened up a new income stream for me. The 'Zero to Hero' title is accurate. Great job, Etubers team!"</p>
                    <p className="text-sm font-semibold text-white">— Elias A. (Digital Creator)</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <footer id="contact" className="bg-[#1A1A2E] border-t border-[#3A3A60] py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-12 mb-6">
                ...
                <p className="text-lg font-bold text-white">Connect with the Etubers Community:</p>

                {/* Social Media Links */}
                <div className="flex space-x-6">
                    {/* YouTube */}
                    <a href="https://www.youtube.com/@etubersmoney" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-red-600 transition duration-300" aria-label="YouTube Channel">
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-youtube"><path d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 2-2h15a2 2 0 0 1 2 2 24.12 24.12 0 0 1 0 10 2 2 0 0 1-2 2h-15a2 2 0 0 1-2-2Z"/><path d="m10 15 5-3-5-3v6Z"/></svg>
                    </a>
                    {/* Telegram */}
                    <a href="https://t.me/etubersmoney" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-500 transition duration-300" aria-label="Telegram Group">
                        <Send className="w-7 h-7" />
                    </a>
                    {/* TikTok (Placeholder for generic social icon) */}
                    <a href="#" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition duration-300" aria-label="TikTok">
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-tiktok"><path d="M21 21h-2a2 2 0 0 1-2-2V9a2 2 0 0 1-2-2h-3v8h3a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2h-3v-6h-2v6a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2v-2a2 2 0 0 1 2-2h3v-8H4a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h11a2 2 0 0 1 2 2v12h4a2 2 0 0 1 2 2Z"/></svg>
                    </a>
                </div>
            </div>
            <p className="text-gray-500 text-sm mt-8">© 2025 Etubers. Powered by the Talk Less, Do More philosophy. | Never Settle.</p>
        </div>
    </footer>
);
